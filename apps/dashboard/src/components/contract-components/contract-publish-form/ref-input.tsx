...
import { AbiParameter } from "abitype";
  ...
  export const RefContractInput: React.FC<RefContractInputProps> = ({
    ...
    const publishedContractsQuery = usePublishedContractsQuery(
      form.watch(`implConstructorParams.${param.name ? param.name : "*"}.ref.publisherAddress`),
    );
  
    const allVersions = useAllVersions(
        form.watch(`implConstructorParams.${param.name ? param.name : "*"}.ref.publisherAddress`),
        form.watch(`implConstructorParams.${param.name ? param.name : "*"}.ref.contractId`),
    );
        ...
        >
          ...
          >
            ...
            <Input
              ...
              {...form.register(`implConstructorParams.${param.name ? param.name : "*"}.ref.publisherAddress`)}
            />
          </FormControl>
          <FormControl as={Flex} flexDir="column" gap={1}>
            ...
            >
              <Select
                ...
                {...form.register(`implConstructorParams.${param.name ? param.name : "*"}.ref.contractId`)}
                placeholder={
          ...
          <FormControl as={Flex} flexDir="column" gap={1}>
            ...
            >
              <Select
                ...
                {...form.register(`implConstructorParams.${param.name ? param.name : "*"}.ref.version`)}
                borderRadius="lg"
              >
